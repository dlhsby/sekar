...
| Status chips horizontally scrollable in sheet | `StatusSummaryBar.tsx` | First tried `BottomSheetScrollView` from `@gorhom/bottom-sheet` — wrong abstraction; that's meant to be the PRIMARY content scroll, not a nested horizontal carousel, and the chips remained unscrollable on Android. Final fix: import `ScrollView` from `react-native-gesture-handler` with `nestedScrollEnabled`. The gesture-handler ScrollView's native gesture coordinates with both the sheet's vertical pan AND the parent `BottomSheetFlatList` host, so horizontal pans pass through cleanly. Added a global jest mock at `__mocks__/react-native-gesture-handler.js` (mapped via `moduleNameMapper`) since the package ships Flow-syntax source jest doesn't transform. |
