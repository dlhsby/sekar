...
Phase 4 completes SEKAR from a production-hardened application (post-Phase 3) into a fully featured, released product. While Phase 3 adds infrastructure (Redis, Sentry, export module, E2E testing), Phase 4 delivers the remaining business features: reporting with PDF/CSV/Excel generation, analytics dashboards with KPIs, asset management with QR codes, iOS platform support, and comprehensive documentation.
...
|---|-------------|--------|-----------|
| 1 | Reporting module with PDF/CSV/Excel export | User | 4-1 |
| 2 | KPI recommendations for worker/area/operational metrics | User | 4-1, 4-2 |
| 3 | Analytics dashboards (web + mobile) | User | 4-2 |
| 4 | Asset management scoped to rayon/area | User | 4-3 |
| 5 | iOS platform support (Apple Sign-In, biometrics, App Store) | User | 4-4 |
| 6 | Multi-environment release coordination (staging + production) | User | 4-5 |
| 7 | User guide for web (markdown) | User | 4-6 |
| 8 | User guide for mobile (markdown) | User | 4-6 |
| 9 | Maintenance documentation | User | 4-6 |
| 10 | Phase 1-4 evaluation framework | User | 4-7 |
| 11 | Suggested features for application completion | User | 4-7 |
| 12 | E2E testing extension for Phase 4 features | User | 4-8 |

---

## Current Codebase Facts (Post-Phase 3 Expected Values)

| Fact | Post-Phase 3 State | Phase 4 Target |
|------|-------------------|----------------|
...
| Documentation | Specs updated in Phase 3-10 | User guides, maintenance docs, evaluation |
...
|-----------|------|--------|-------------|--------------|
| **4-1** | Reporting Module | 10-13 days | Phase 3 ExportModule | #1, #2 |
| **4-2** | Analytics Module | 8-10 days | Phase 3 Redis cache | #2, #3 |
| **4-3** | Asset Management | 8-10 days | Phase 3 infrastructure | #4 |
| **4-4** | iOS Platform | 8-10 days | Independent | #5 |
| **4-5** | Release & Deployment | 5-7 days | 4-1..4-4 complete | #6 |
| **4-6** | User Guides & Documentation | 5-7 days | 4-1..4-5 complete | #7, #8, #9 |
| **4-7** | Evaluation & Final Polish | 4-5 days | All complete | #10, #11 |
| **4-8** | E2E Testing Extension | 5-7 days | 4-1..4-4 features | #12 |
...
```
Week 1-2:   4-1 (Reporting) ─────────────────────┐
            4-2 (Analytics) ─────────────────────┤  <- Parallel
                                                  |
Week 3-4:   4-3 (Asset Management) ──────────────┤
            4-1 (Report scheduler + tests)       |
                                                  |
Week 5-6:   4-4 (iOS Platform) ──────────────────┤  <- Independent
            4-5 (Release prep, staging) ─────────┤
                                                  |
Week 7-8:   4-8 (E2E Testing) <── 4-1..4-4      |
            4-6 (User Guides) <── 4-1..4-5       |
Week 9:     4-7 (Evaluation) <── All ────────────┘
```

### Sub-Phase Dependency Notes

- **4-1 (Reporting) extends Phase 3 ExportModule:** The ExportModule from Phase 3 (sub-phase 3-5) provides CSV/Excel generation. Phase 4 adds Puppeteer PDF, report templates, scheduled reports, and report archive. Do NOT duplicate export infrastructure — extend `export.service.ts` and add `reporting.module.ts` as a consumer.
- **4-2 (Analytics) depends on Phase 3 Redis:** Analytics materialized views use Redis for caching aggregated metrics (TTL 5min). Without Redis, analytics queries hit raw tables and may exceed 2s response threshold.
- **4-4 (iOS) is fully independent:** Can start any time after Phase 3. No backend or database dependencies beyond existing auth system.

---

### 4-1: Reporting Module (10-13 days)
...
### 4-2: Analytics Module (8-10 days)
...
### 4-3: Asset Management (8-10 days)
...
### 4-5: Release & Deployment (5-7 days)
...
### 4-6: User Guides & Documentation (5-7 days)

**Requirements:** #7 web guide, #8 mobile guide, #9 maintenance docs

| Task | Scope | Key Files |
|------|-------|-----------|
| A1. Web user guide | 14 sections by role | `specs/phases/phase-4-finishing/user-guide-web.md` |
| A2. Mobile user guide | 9 sections by role | `specs/phases/phase-4-finishing/user-guide-mobile.md` |
| A3. Maintenance documentation | 10 sections for system admin | `specs/phases/phase-4-finishing/maintenance.md` |
| A4. API documentation update | Swagger annotations for new endpoints | `be/src/modules/*/` |
| A5. Architecture diagrams update | C4 diagrams with Phase 4 additions | `specs/architecture/` |

**Deliverables:** Web user guide, mobile user guide, maintenance manual, updated API docs

---

### 4-7: Evaluation & Final Polish (4-5 days)

**Requirements:** #10 evaluation, #11 suggested features

| Task | Scope | Key Files |
|------|-------|-----------|
| A1. Cross-phase evaluation | Phase 1-4 requirements vs delivered | `specs/phases/phase-4-finishing/evaluation.md` |
...
### 4-8: E2E Testing Extension (5-7 days)
...
| Current Code | Change |
|-------------|--------|
| `be/src/modules/export/` (Phase 3) | Extended with Puppeteer PDF generator |
| `@nestjs/schedule` cron jobs (Phase 3) | +3 cron jobs (report scheduler, analytics refresh, maintenance reminders) |
...
Per user decision, Phase 4 focuses on core features. These are documented as Phase 5 candidates:
...
> **Note:** These features are NOT included in Phase 4 scope or effort estimates. They require separate approval and planning.
