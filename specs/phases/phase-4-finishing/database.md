...
**Date:** March 13, 2026
**Status:** Not Started
**Depends On:** Phase 3 Database (Complete — 24 tables, 8 migrations)
**Related Sub-Phases:** 4-1, 4-2, 4-3
...
| Fact | Value |
|------|-------|
| Tables | 24 (Phase 2E: 22 + Phase 3: notification_preferences, export_jobs, location_daily_summaries) |
| Migrations | 8 (6 from Phase 2E + Phase3Tables + Phase3Indexes) |
| ORM | TypeORM with auto-sync in dev, migrations in production |
| Indexes | Composite indexes added in Phase 3 (location_logs, shifts, activities, tasks, overtimes, audit_logs, user_tracking_status) |
| Export | export_jobs table (Phase 3) with CSV/Excel support |
...
## A. New Tables (Sub-Phase 4-1: Reporting)
...
## B. New Tables (Sub-Phase 4-3: Asset Management)
...
## C. Materialized Views (Sub-Phase 4-2: Analytics)
...
### D1. Migration A — Reporting Tables (Sub-Phase 4-1)
...
### D2. Migration B — Asset Tables (Sub-Phase 4-3)
...
### D3. Migration C — Analytics Materialized Views (Sub-Phase 4-2)
...
## D-extra. Migration D — Apple Sign-In Column (Sub-Phase 4-4)

**File:** `be/src/database/migrations/1743000003000-Phase4AppleSignIn.ts`

> **Note:** Adds `apple_id` column to `users` table for Apple Sign-In (iOS). Independent of other Phase 4 migrations.
...
|---|-------|------|-----------|---------|
| 1-24 | (Phase 3 tables) | - | - | `users` +apple_id column (4-4) |
| 25 | **report_templates** | **NEW** | 4-1 | 6 built-in report templates |
| 26 | **report_schedules** | **NEW** | 4-1 | Cron-based report scheduling |
| 27 | **generated_reports** | **NEW** | 4-1 | Report archive with S3 storage |
| 28 | **asset_categories** | **NEW** | 4-3 | 6 park-specific categories |
| 29 | **assets** | **NEW** | 4-3 | Asset registry with QR codes |
| 30 | **asset_assignments** | **NEW** | 4-3 | Checkout/return tracking |
| 31 | **asset_maintenances** | **NEW** | 4-3 | Maintenance scheduling |
...
## H. Retention Policy (Phase 4 Additions)
