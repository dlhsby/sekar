...
- ✅ iOS `LaunchScreen.storyboard` (pinwheel `UIImageView` above the SEKAR label, `SekarPinwheel.imageset`) + Android `res/drawable/bootsplash.xml` → `splash_logo.png` — both show the box-lockup on the sage canvas (2026-05-28).
- `react-native-bootsplash` config update for new splash
- `SekarLogoBox` is the canonical SEKAR mark (white NB box + pinwheel, tilt prop) — used in Login (with `SEKAR` wordmark), `SplashSlide`, the app icon, and the native splash.
