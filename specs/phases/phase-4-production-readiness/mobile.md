...
| TUG-1 | Tugas list · filtered tabs | `fe/mobile/src/screens/taskActivity/TasksActivityScreen.tsx` | ✅ Revamped (2026-05-28) — `TaskCard` rebuilt on `HomeListRow` + `StatusPill` (dotted pill, mirrors TodayTasksModal); `taskPill` extended to all 8 statuses; right-meta = in_progress elapsed / deadline / created; subMeta = area · assignee. `TasksTab` states: error→`NBEmptyState` illo-offline + retry, filtered-empty→illo-search (new `isFiltered` prop). Prior token-shim pass 2026-05-26 |
