...
| PRT-2 | Review queue · Admin Data | ReviewQueueScreen (pruning-requests list, admin filter) | Revamp — rows use the CP1 shared `PerantinganRequestCard`; shared `PruningRequestFilterModal` already swept in CP2 (2026-06-08); screen token-sweep + SLA `extraTag` pill = CP3 |
