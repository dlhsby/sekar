...
- **Manifest fix:** `ACCESS_BACKGROUND_LOCATION` was **missing** from `AndroidManifest.xml` — so background location could never be granted and the OS never offered "Allow all the time" (the reported bug). Now declared.
- **Background location is OPTIONAL** (tagged "· Opsional") and does **not** gate "Lanjut" — Android 11+ grants "all the time" only via Settings, which is poor onboarding UX for satgas. Its "Izinkan" requests foreground location first, then background **inline (no Settings dump)**; an `AppState` foreground re-check (`checkAllPermissions`) upgrades the pill to DIBERIKAN if the user later allows it from Settings. The app can re-prompt at clock-in where background tracking is actually used.
- **Required (gate "Lanjut"):** notifikasi · lokasi · kamera · galeri (all one-tap).
