...
**M3 revamp status — satgas Home restructure + bottom-sheet revamp ✅ (2026-05-26) — Checkpoint 7:** second manual-review round. (1) Coordinator team grid now uses a **uniform coordinator-role tint** (not per-member). (2) Active-hero **clock shrunk + single-line** (`adjustsFontSizeToFit`) — fixes the wrapping second-digit group. (3) **FieldHome (HOME-1) restructured**: "Ringkasan hari ini" moved directly under the hero; the inline **"Tugas hari ini" list removed** — the Ringkasan **Tugas tile now opens a bottom sheet** (mirrors Aktivitas/Jam kerja). (4) **Bottom sheets revamped to v2.1**: new `TodayTasksModal` + `TodayActivitiesModal`/`TodayWorkHoursModal`/`ShiftCard` rebuilt on `NBModal` + `NBText` + tokens (dropped legacy `nbTypography`/`borderRadius:0`/emoji empty states). Full suite 4195 passing.

**M3 revamp status — Home manual-review polish ✅ (2026-05-26) — Checkpoint 6:** acting on the first round of manual feedback on the Home work. (1) New shared **`RoleAvatar`** primitive (`components/common/`) — role-accent tint + black initials, **renders `profile_picture_url` when set** else initials; auto-scales to box size. Now used by both the masthead and the Coordinator team grid (grid avatars are role-tinted instead of plain white). (2) Masthead **role-label/name fonts reduced** (role 12→10, name 18→15) so the `· {area}` suffix + longer names show more before tail-ellipsis. (3) Masthead right slot **reordered**: status chip now LEFT of the bell (bell rightmost). (4) The active **"Sedang bertugas" hero is collapsible** (whole card toggles open/closed via chevron; expanded reveals the Clock-out button + "Mulai…" meta + a "Detail shift →" link) — **restores the collapsibility dropped in Checkpoint 2b**, now on v2.1 tokens. Tokens-only; `no-inline-hex-colors` clean. Tests: RoleAvatar 8 + masthead photo + FieldHome collapse-toggle; home/nav suites green (104).
