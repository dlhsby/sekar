...
**Date:** March 12, 2026 (updated April 25, 2026 — items promoted into Phase 3 noted)
**Status:** Not Started
**Priority:** High — Pre-Production Hardening
**Duration:** 52-68 developer-days estimated (7-9 weeks)
**Depends On:** Phase 3 (Complete) — inherits Redis 7 stack, generated-token pipeline, web PWA shell, NB design system migration, mobile-web responsive layouts
...
## ⚠️ Items Promoted Into Phase 3 (no longer in Phase 4 scope)

The following items were originally Phase 4 scope but have been promoted into Phase 3 M1-R (Apr 25, 2026):

| Item | Original Phase 4 home | Now in |
|------|----------------------|--------|
| Redis 7 infrastructure (caching, rate-limiting, JWT blacklist) | 4-1 Infrastructure | Phase 3 sub-phase 3-3 (Redis Streams + Socket.IO Redis adapter) — already installed when Phase 4 starts |
| Web PWA (manifest, service worker, install prompt, offline shell, push subscription) | 4-8 UI/UX Polish (web production readiness item #17) | Phase 3 M1-R sub-phase **3-R4** |
| Design-token migration sweep (NB primitives + login/dashboard/users/areas/rayons/overtime/schedules/reports/profile/settings/audit-logs onto generated tokens) | implicit Phase 4 cleanup | Phase 3 M1-R sub-phase **3-R5** (full sweep — no screen left on old tokens) |
| Mobile-web responsive layouts at 375 / 768 / 1280 px | implicit Phase 4 cleanup | Phase 3 M1-R sub-phase **3-R4** + 3-R5 |
| Brand-font bundling (Space Grotesk + Inter + JetBrains Mono on both platforms) | implicit Phase 4 cleanup | Phase 3 M1-R sub-phase **3-R2** |
| New mobile NBModal / NBToast / NBText components | implicit Phase 4 component-library expansion | Phase 3 M1-R sub-phase **3-R3** |

**Phase 4 net effect:** when Phase 4 begins, Redis is already running, the design system is on generated tokens repo-wide, the web is an installable mobile-responsive PWA, and visual regression CI is gating PRs. Phase 4 focuses only on production-hardening items that did NOT need redesign-system foundations: offline sync polish, FCM full activation, exports, security hardening, refactoring, observability, mobile production readiness (ProGuard, crash reporting, deep linking), bundle analysis, KMZ web import, real-data seeder, task-status simplification (ADR-009 8→4 debt). Items #17 web PWA and #12 UI/UX polish (design-system parts) collapse from "implement" to "verify and polish what M1-R shipped".

---

## Overview

Phase 4 transforms SEKAR from a feature-complete application into a production-hardened system. While all core features exist (18 modules, ~130 endpoints, 22 mobile screens, 24+ web pages), the codebase lacks production infrastructure: no Redis caching, no structured logging, no export functionality, incomplete offline sync, FCM wired but not activated, no E2E testing, and security gaps (no refresh tokens, no rate limiting per endpoint).
...
| 2 | Push notification (FCM full activation) | User | 4-3 |
| 3 | Refactor (SOLID, five-lines-of-code, reusable components) | User | 4-7 |
| 4 | Optimization on all features | User | 4-7 |
| 5 | WebSocket evaluation | User | 4-1 |
| 6 | Message broker (if needed) | User | 4-1 |
...
| 12 | UI/UX polish for all (mobile and web) | User | 4-8 |
...
| 17 | Web production readiness (SEO, PWA, bundle analysis) | Additional | 4-8 |
...
| Redis | **Installed in Phase 3 sub-phase 3-3** (Streams + Socket.IO adapter for monitoring) | Phase 4 adds caching, rate-limiting, JWT blacklist, notification retry queue uses on the same Redis 7 instance |
...
|-----------|------|--------|-------------|--------------|
| **4-1** | Infrastructure & Evaluation | 4-5 days | None | #5, #6, #14 |
| **4-2** | Offline Sync Completion | 4-5 days | 4-1 (health endpoint) | #1, #18 |
| **4-3** | Push Notification Activation | 5-6 days | 4-1 (Redis retry queue) | #2, #19 |
| **4-4** | Worker Reassignment Workflow | 3-4 days | None | #7, #19 |
| **4-5** | Export & Import Data | 5-7 days | None | #9, #10 |
| **4-6** | Real Data Seeder & Data Management | 4-5 days | 4-5 (CSV import) | #8, #15 |
| **4-7** | Refactor, Optimization & Security | 12-15 days | 4-1 (Redis) | #3, #4, #13, #18 |
| **4-8** | UI/UX Polish & Production Readiness | 5-7 days | None | #12, #16, #17 |
| **4-9** | E2E Testing | 7-8 days | 4-2 through 4-8 | ADR-017 |
| **4-10** | Documentation Sync | 2-3 days | All | #11 |

**Total estimated:** 52-68 developer-days

> **Note:** Estimates revised based on expert architectural review (March 2026). 4-7 increased from 7-9 to 12-15 days (3 service extractions + JWT refresh + rate limiting + timezone audit). 4-9 increased from 5-6 to 7-8 days (CI pipeline setup with KVM and emulator takes 2-3 days).

### Parallelization Opportunities

```
Week 1-2:  4-1 (Infrastructure) ──────────────┐
           4-2 (Offline Sync) ←── start early │   ← 4-4 depends on 4-2 (offlineQueue.ts)
           4-5 (Export/Import) ─────────┐     │
                                       │      │
Week 2-3:  4-2 (Offline Sync) ←───────────── 4-1
           4-3 (Push Notifications) ←──────── 4-1
           4-6 (Data Mgmt) ←────────── 4-5    │
           4-4 (Reassignment) ←── 4-2 merged  │   ← start AFTER 4-2 offlineQueue changes
                                               │
Week 3-5:  4-7 (Refactor/Security) ←──────── 4-1
           4-8 (UI/UX Polish) ─────────────────┤
                                               │
Week 6-8:  4-9 (E2E Testing) ←─── 4-2..4-8   │
           4-10 (Doc Sync) ←─── All ──────────┘
```

### Sub-Phase Dependency Notes

- **4-4 (Reassignment) has a soft dependency on 4-2 (Offline Sync):** Both sub-phases modify `offlineQueue.ts`. If run in parallel, merge conflicts are likely. **Recommendation:** Start 4-2 first; begin 4-4 only after 4-2's offlineQueue changes are merged.
- **4-1 (Infrastructure) → 4-7 (Refactoring) sequencing for Redis adapter:** Redis is **already installed in Phase 3 sub-phase 3-3** with Socket.IO adapter active for the monitoring `monitoring:` rooms. In Phase 4, the adapter is extended to non-monitoring WebSocket events; that extension cannot complete until `emitToUser()` is refactored to room-based emit in sub-phase 4-7 (see ADR-016 HARD DEPENDENCY). 4-1 reuses the Redis 7 instance for caching/rate-limiting/JWT blacklist; 4-7 finishes the emit refactor so the Redis adapter can fan out every WS event horizontally.

---

### 4-1: Infrastructure & Evaluation (4-5 days)
...
### 4-2: Offline Sync Completion (4-5 days)
...
### 4-3: Push Notification Activation (5-6 days)
...
### 4-4: Worker Reassignment Workflow (3-4 days)
...
### 4-5: Export & Import Data (5-7 days)
...
### 4-6: Real Data Seeder & Data Management (4-5 days)
...
### 4-7: Refactor, Optimization & Security (12-15 days)
...
### 4-8: UI/UX Polish & Production Readiness (5-7 days)
...
### 4-10: Documentation Sync (2-3 days)
...
| C4. System overview update | Update `specs/architecture/system-overview.md` to reflect Phase 4 additions (Redis, Sentry, export module, health endpoints, new tables) | `specs/architecture/system-overview.md` |
