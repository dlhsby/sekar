...
**AS-4 Forgot password.** `ForgotPasswordScreen` rebuilt to hi-fi AS-4: app-bar (back + "Lupa Kata Sandi"), lock-icon hero, rayon contact list, dashed temp-password note, "Kembali ke Login". New reusable `ContactChannelCard` (WhatsApp/phone icon card, `tel:`/`wa.me` links). Reconciliation: kept the **dynamic per-rayon** contact list (real feature) over the hi-fi's two fixed channels; WhatsApp green → `statusActive` token (no inline hex).

**AS-5 + AS-5b Change password.** `ChangePasswordScreen` rebuilt: `NBAlert` banner, three password fields, **live** `RequirementChecklist` (min 8 / huruf+angka / beda dari sementara / konfirmasi cocok) gating "Simpan & Masuk", and "Keluar & login lain" (clearAll + `logout`). On success shows the AS-5b `SuccessOverlay` (check circle + pulsing dots) ~1.5s before RootNavigator routes. New reusable `RequirementChecklist` + `SuccessOverlay`.
