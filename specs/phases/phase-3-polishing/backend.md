# Phase 3: Backend Specifications

**Date:** March 12, 2026
**Status:** Not Started
**Depends On:** Phase 2E Backend (Complete)
**Related Sub-Phases:** 3-1, 3-3, 3-5, 3-6, 3-7
...
## B. Structured Logging (Sub-Phase 3-1)
...
## C. FCM Activation (Sub-Phase 3-3)
...
## D. Notification Preferences (Sub-Phase 3-3)
...
## G. Security Hardening (Sub-Phase 3-7)
...
- Phase 3: keep 7d expiry as fallback, add refresh token infrastructure
- Phase 3 + 1 week: mobile app update deployed with refresh token support
- Phase 3 + 2 weeks: switch access token to 15m, enable rotation
...
## G6. New Error Codes (Phase 3)

The following error codes are introduced in Phase 3. Add them to `specs/api/error-handling.md` during sub-phase 3-10 (final cleanup pass).
...
## H. Refactoring Targets (Sub-Phase 3-7)
...
- This MUST be done BEFORE activating Redis adapter in sub-phase 3-1
...
## I. Data Management (Sub-Phase 3-6)
...
## J. Production Seeder (Sub-Phase 3-6)
...
## K. Redis Integration (Sub-Phase 3-1)
...
## L. Testing Guidance for Phase 3 Features
...
`ioredis-mock` is in-memory and supports `SET`, `GET`, `DEL`, `EXISTS`, `SET NX EX`, TTL semantics — sufficient for all Phase 3 Redis usage patterns. No real Redis instance required in CI.
...
- New Phase 3 modules: >90% statement coverage target
