...
**Purpose:** Comprehensive deployment procedures, environment setup, and manual testing checklist for Phase 3 verification.
...
### 2. Environment Variables (New in Phase 3)
...
# FCM Activation (Sub-Phase 3-3)
FCM_ENABLED=true                          # Was false
FCM_SERVICE_ACCOUNT_PATH=/path/to/firebase-service-account.json

# Rate Limiting (Sub-Phase 3-7)
...
|-------------|-----------|---------------|---------------------|
| `FCM_ENABLED` | 3-3 | `false` | Set `true` after all 8 triggers wired + preferences table seeded |
| Redis adapter | 3-1 | Disabled | Enable after `emitToUser()` refactored to room-based pattern |
| JWT refresh | 3-7 | Disabled | Enable after mobile/web clients deployed with refresh support |
| PWA service worker | 3-8 | Disabled | Enable after cache scope exclusions tested |
| Per-endpoint rate limits | 3-7 | Global only | Enable after load testing confirms thresholds |
...
### Section 1: Infrastructure (Sub-Phase 3-1)
...
### Section 2: Offline Sync (Sub-Phase 3-2)
...
### Section 3: Push Notifications (Sub-Phase 3-3)
...
### Section 4: Worker Reassignment (Sub-Phase 3-4)
...
### Section 5: Export & Import (Sub-Phase 3-5)
...
### Section 6: Data Management (Sub-Phase 3-6)
...
### Section 7: Security & Refactoring (Sub-Phase 3-7)
...
### Section 8: UI/UX Polish (Sub-Phase 3-8)
...
### Section 9: E2E Testing (Sub-Phase 3-9)
...
### Section 10: Documentation (Sub-Phase 3-10)

- [ ] `COMPLETION_STATUS.md` updated with Phase 3 metrics
- [ ] `system-overview.md` reflects Phase 3 additions (Redis, Sentry, export, health)
...
|-----------|-------------------|------------|
| 3-1 (Infrastructure) | Remove Redis config, revert to in-memory; disable Sentry DSN | Low |
| 3-2 (Offline Sync) | Revert to binary online/offline; remove new queue types | Low |
| 3-3 (Push Notifications) | Set `FCM_ENABLED=false`; drop notification_preferences table | Low |
| 3-4 (Reassignment) | Disable reassignment UI; audit logs remain | Low |
| 3-5 (Export/Import) | Disable export endpoints; drop export_jobs table | Low |
| 3-6 (Data Management) | Stop cron jobs; keep existing data | Low |
| 3-7 (Refactor/Security) | **HIGH RISK** — JWT change is breaking; keep 7d expiry if rollback needed | High |
| 3-8 (UI/UX) | Cosmetic only; no data risk | Low |
| 3-9 (E2E Testing) | No production impact — CI only | None |
| 3-10 (Documentation) | No production impact | None |
...
# Environment: Remove Phase 3 env vars
...
*Phase 3 Production Readiness: Deployment & Testing Checklist*
*Last Updated: March 12, 2026*
