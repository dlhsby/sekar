...
| `POST` | `/api/v1/pruning-requests/:id/assign-to-task` | same | `{ areaId?, assignedTo, scheduledDate?, caseType?, pruningAction?, units? }` → returns `{ request, task }`. **May 1, ADR-035:** `scheduledDate` optional (auto-pick first available day in preferred week). **May 11, 2026:** `areaId` optional (pruning runs outside managed areas; task inherits `rayon_id` + GPS + address from the request). `units` defaults to `1`. **May 11 late+1:** `caseType` + `pruningAction` are now also optional — admins no longer pre-classify the work; the satgas captures the actual case + action on the activity report. Mobile form is reduced to two-step assignee picker (Peran → Person) + read-only scheduled-date chip pre-filled from `request.scheduledDate`. Assignee whitelist: `korlap`, `satgas`, `linmas`, `kepala_rayon`, `admin_data` (admin may pick themselves for the centralized-recap pattern). Status transitions to `assigned`. |
