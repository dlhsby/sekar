...
## Changelog
- 2026-07-26 — **Standard screen layout codified + swept app-wide.** Added `src/constants/layout.ts` (`screenContent` / `screenContentGrow`) as the canonical page padding + gave `NBEmptyState` a `compact` prop for in-page (non-full-screen) empties. **Swept ~44 screens** onto the constant (all role home dashboards, attendance/clock-in, lists, forms, detail screens, tabs), preserving each screen's list `gap` and FAB/keyboard/safe-area `paddingBottom` overrides; horizontal chip rows, modals/sheets and calendar components were intentionally left. `ClockInOutScreen` keeps its no-horizontal-padding body (its cards own horizontal spacing). See [`component-library.md` §Standard screen layout](component-library.md). New screens must use `screenContent` rather than hand-rolled padding.
