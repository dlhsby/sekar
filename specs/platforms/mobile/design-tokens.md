...
> **Mobile-platform lens on canonical tokens (Phase 3 M1-R +).** This document shows how Layer-1 tokens from `specs/design-system/tokens.json` surface inside React Native. Any value MUST match the canonical source. To change a token: edit `tokens.json`, run `npm run tokens:build`, commit the regenerated `apps/mobile/src/constants/generated/tokens.ts`. Never hand-edit `nbTokens.ts` or this document — CI (`npm run tokens:verify`) will reject drift.
...
**Single source of truth:** [`specs/design-system/tokens.json`](../../design-system/tokens.json) (validated by [`tokens.schema.json`](../../design-system/tokens.schema.json))
**Canonical registry:** [`specs/design-system/design-tokens.md`](../../design-system/design-tokens.md)
...
1. Edit [`specs/design-system/tokens.json`](../../design-system/tokens.json).
...
> **Phase-3 override (ADR-036):** The generator emits `shadowRadius: 0, shadowOpacity: 1, elevation: max(offsetX, offsetY)` regardless of the soft-edge values shown below. The Phase-2 soft-edge shape is retained here as historical reference for consumers migrating from `nbTokens.ts`. See [specs/design-system/design-tokens.md §Shadows](../../design-system/design-tokens.md) for the canonical hard-edge emission.
...
> **Phase-3 override (ADR-036):** The v2.0 → v3.0 transition **reverts** to hard-edge shadows. The generator emits `shadowRadius: 0, shadowOpacity: 1, elevation: max(X,Y)` regardless of the values documented below. The v1.0 → v2.0 migration table is kept for historical context only. See [specs/design-system/design-tokens.md §Shadows](../../design-system/design-tokens.md).
