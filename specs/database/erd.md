...
| Korlap | `korlap` | 1 Area | Area coordination (via location_id) |
...
|-------------|--------|-------|------|------------|-------|
| Rayon-Area | rayons | locations | 1:inf | FK(rayon_id) | 7 rayons, many areas each |
| Rayon-User | rayons | users | 1:inf | FK(rayon_id) | kepala_rayon role |
| Rayon-Task | rayons | tasks | 1:inf | FK(rayon_id) | Rayon-scoped tasks |
| LocationType-Area | location_types | locations | 1:inf | FK(location_type_id) | ACTIVE/PASSIVE category |
| Area-User | locations | users | 1:inf | FK(location_id) | korlap role |
| User-Schedule | users | schedules | 1:inf | FK(user_id) | Primary assignment |
| Area-Schedule | locations | schedules | 1:inf | FK(location_id) | Schedule location |
...
| Area-Shift | locations | shifts | o:inf | FK(location_id) | Nullable |
...
| Area-Task | locations | tasks | o:inf | FK(location_id) | Nullable for rayon-scoped |
...
| Area-Overtime | locations | overtimes | o:inf | FK(location_id) SET NULL | Location |
...
| Area-StaffReq | locations | location_staff_requirements | 1:inf | FK(location_id) CASCADE | Requirements |
...
| users.location_id | SET NULL | User persists if area deleted |
| schedules.user_id | CASCADE | Remove schedules when user deleted |
| schedules.location_id | CASCADE | Remove schedules when area deleted |
...
| tasks.location_id | RESTRICT | Prevent deletion of area with tasks |
...
| overtimes.location_id | SET NULL | Overtime persists if area deleted |
...
| location_staff_requirements | uq_area_staff_requirements | (location_id, shift_definition_id, role, day_type) |
...
|-------|--------|---------------------|
| Phase 1 (Core) | 7 | users, location_types, areas, worker_assignments, shifts, work_reports, location_logs |
| Phase 2A (Rayons) | 5 | rayons, shift_definitions, worker_schedules, location_staff_requirements, special_day_overrides |
...
- **korlap** → assigned via `user_locations` (permanent, multiple areas in 1 rayon); `users.location_id` kept for backward compat
...
| Relationship | Parent | Child | Type | Constraint | Notes |
|-------------|--------|-------|------|------------|-------|
| User-UserArea | users | user_locations | 1:inf | FK(user_id) CASCADE | Multi-area assignment |
| Area-UserArea | locations | user_locations | 1:inf | FK(location_id) CASCADE | Area assignment |
...
|-------|--------|-------------|
| Phase 2E (Feedback II) | +2 | +user_locations, +audit_logs |
...
| Phase | Tables | New/Changed |
|-------|--------|-------------|
| Phase 3 (Plants/Monitoring Rebuild) | +8 | +plant_species, +location_plants, +notable_plants, +activity_plant_items, +pruning_requests, +service_capacity, +plant_seeds, +seed_transactions |
