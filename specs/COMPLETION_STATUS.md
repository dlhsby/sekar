...
| **Phase 3 Implementation** | 🟡 In Progress | ~48% (10/21 sub-phases) | all green (mobile 3,836 total; see per-component rows) | — | Plants + Monitoring Rebuild + Public Intake. M1-R ✅ + 3-1 ✅ + 3-2 ✅ + **3-3 ✅ (Apr 26, backend):** `RedisService`, `StatusProjectorService`, `StaffingDebouncerService`, `StaleStatusSweeperService`, `GET /monitoring/snapshot`, Socket.IO Redis adapter, `CONCURRENTLY` location_logs indexes; 34 new tests. **3-4 ✅ (Apr 26, web):** `ClusterLayer` (supercluster), `WorkerListVirtual` (@tanstack/react-virtual), `HierarchyFilterPanel`, `AreaDetailDrawer`, `status:v2` WS cache patch, `staff_kecamatan` nav + `(kecamatan)` layout; 10 new tests. **3-5 ✅ (Apr 26, mobile):** `monitoringV2Slice` (31 tests), `ClusterMarker` (11), `ClusteredUserMarkers` (9), `MonitoringToggleSheet` (10), `AreaStatusOverlay`, `PlantOverlayLayer` (stub), `featureFlags.clusterMarkersV2=false`, ESLint ban; 70 total new tests. **M2 milestone complete. All 159 mobile suites green after Apr 26 test-fix session. Next: 3-6 task typing + partial-complete API.** |
