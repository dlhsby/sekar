...
It walks the four scenarios end-to-end — **run locally → obtain keys → deploy to staging → deploy to production** — and links to focused deep-dive guides for exhaustive detail. Historical per-phase deployment records were retired in the docs restructure — the build timeline lives in [`../history/CHANGELOG.md`](../history/CHANGELOG.md).
...
This brings up local infra (PostgreSQL :5432, Adminer :8080, MinIO S3 :9000 + console :9001, Redis :16379) and runs the backend (`http://localhost:3000`, API docs `/api/v1/docs`) and web (`http://localhost:3001`). Test users use `12345678` (e.g. `admin/12345678`, `satgas1/12345678`).
...
```
> `db:seed:staging:prod` is the staging dataset (idempotent, all passwords `12345678`).
...
```
Then log in (`superadmin/12345678`) and confirm the monitoring map renders (Google Maps +
...
- Build history: [`../history/CHANGELOG.md`](../history/CHANGELOG.md)
