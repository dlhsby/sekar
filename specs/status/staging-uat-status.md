...
| **Secrets** | ✅ dotenvx | encrypted `apps/be/.env.staging` baked into the image; only the private key (`DOTENV_PRIVATE_KEY_STAGING`) is pulled from SSM `/sekar/staging/BE_DOTENV_PRIVATE_KEY` into `/opt/sekar/.env` at deploy |
