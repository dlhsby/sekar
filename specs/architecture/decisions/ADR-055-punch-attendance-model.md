...
- **Koreksi Kehadiran is DROPPED** (2026-07-28) — not deferred, not planned. An approval-gated correction overlay needs an approver, and there is none: satgas/linmas/korlap carry no `users.rayon_id` (their rayon is schedule-derived, ADR-053) and korlap's tie to a worker is schedule-derived too, so every candidate routing is circular. Rather than carry a design nobody can staff, v1 has **no approval subsystem at all** and the feature is out of scope until someone can name the approver. A punch therefore stays immutable and a forgotten clock-out is **never auto-closed** — it is a flagged `lupa_clock_out`, visible in the log. What changed instead: past the shift's `cutoff_grace_min` that worker stops reading `bertugas` (they become `pulang`, flag retained), so a missing punch no longer holds them on the monitoring map or inside the staffing count. The record stays honest; the live view stops lying.
