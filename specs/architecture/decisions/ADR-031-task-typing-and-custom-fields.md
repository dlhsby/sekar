...
- **2026-05-23 audit note — registry validator not yet enforced at the API boundary.** `tasks.custom_fields` and `activities.custom_fields` columns currently accept any JSON. The `TaskTypeRegistry` design is documented below, but no schema-aware validator runs inside the DTO pipeline today. Tracked in the Phase 3 gap audit (history/CHANGELOG.md, finding M7); fix is Wave 6 hardening, not a Wave 1 blocker.
