...
Field work is often organized as crews — perawatan (maintenance), penyiraman (watering), penanaman (planting), penyapuan (sweeping), etc. UAT wants to **create a team, schedule it as a unit, and monitor it as one group bubble** on the map that drills down to individual members. There is no team concept in the codebase today; workers are grouped only implicitly by rayon + assigned areas.
...
- **`teams`** — `id`, `name`, `team_type_id` (FK), `marker_icon`, `marker_color`, `is_active`, audit. Managed via a Teams CRUD page gated by `team:*` permissions.
...
**Member scope:** the invite picker is filtered to eligible roles **within the PIC's rayon** — all members of a given team schedule belong to the same rayon (matching the region/area the event is scoped to). Cross-rayon teams are not allowed.
...
A team schedule is **static** (all members share an `area_id`) or **mobile** (share a `region_id`), per the event's scope. Mobile teams (e.g. penyiraman) are geofenced to the region.
