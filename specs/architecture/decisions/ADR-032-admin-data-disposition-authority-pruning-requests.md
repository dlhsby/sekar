# ADR-032: Extend `admin_rayon` with Disposition Authority over `pruning_requests`, Scoped by `users.rayon_id`
...
A natural instinct was to introduce a new `admin_rayon` role. The client rejected this: they don't want another role to administer, and the existing `admin_rayon` role is already rayon-scoped via `users.rayon_id` (ADR-013 — multi-area assignment + rayon binding).

Current state (evidence):

- **`admin_rayon` is rayon-scoped**: `users.rayon_id` already binds `admin_rayon` users to a single rayon; `MONITORING_RAYON` permission group (`apps/be/src/modules/users/constants/role-groups.ts`) grants rayon-level monitoring visibility.
- **`admin_rayon` has no approval authority today**: ADR-009 explicitly excludes `admin_rayon` from `OVERTIME_APPROVERS` and similar decision groups. It is a data-entry role.
- **`kepala_rayon` is executive**, not operational. Filtering the daily inbox of kecamatan requests does not match its remit.
- **`management` is city-wide**, not rayon-scoped. It should see the pipeline but not triage every request.

The decision is: narrowly extend `admin_rayon` with one new capability, scoped to one entity, or create a role.

## Decision

Grant `admin_rayon` **disposition authority over `pruning_requests` only**, scoped to requests whose resolved `rayon_id` matches the acting user's `users.rayon_id`. No new role. No other capability changes.
...
```ts
export const PRUNING_REQUEST_REVIEWERS = ['admin_rayon', 'management'] as const;
```
...
// Exception: management bypasses the rayon check (city-wide)
...
- **Matches operational reality.** `admin_rayon` staff already work at rayon level, already handle daily data tasks. Triaging kecamatan requests is a natural extension of that work.
- **`management` inclusion** handles edge cases (cross-rayon reassignment, escalation) without requiring a separate escalation role.

### Negative

- **Subtle dual-capability.** `admin_rayon` now has both data-entry and disposition rights, but only for `pruning_requests`. This is a genuine deviation from ADR-009's "clean separation" principle. Mitigation: this ADR is the sole exception; any future extension of `admin_rayon`'s approval surface requires a new ADR.
...
1. **New `admin_rayon` role.** Rejected by the client. Adds administrative overhead (hire/train/offboard another role) and duplicates ~90% of `admin_rayon`'s capabilities.
...
4. **City-wide `admin_rayon` disposition without rayon scoping.** Rejected. Surabaya's 7 rayons have distinct operational owners; city-wide authority concentrates risk and breaks the rayon-autonomy model established in ADR-013.
...
- [ADR-009](./ADR-009-phase2c-role-system-overhaul.md) — 8-role system; amended by this ADR to note the `admin_rayon` extension
- [ADR-013](./ADR-013-multi-area-assignment.md) — `users.rayon_id` scoping reused here
- [ADR-033](./ADR-033-staff-kecamatan-role.md) — the counterparty role that submits requests for `admin_rayon` to review
- [ADR-031](./ADR-031-task-typing-and-custom-fields.md) — the converted task carries `task_type='pruning'`
- [ADR-038](./ADR-038-pruning-workflow-entry-points.md) — `admin_rayon`'s disposition endpoint is one of five pruning entry points; converted tasks emit a `task_delegations` audit row on every assignment
...
`admin_rayon` disposition (`POST /pruning-requests/:id/review` + `POST /pruning-requests/:id/convert-to-task`) remains rayon-scoped per the rules above. Once the request converts into a task, the onward path **may pass through several roles before execution** — typical chain is `management → kepala_rayon → admin_rayon → korlap → satgas`, but any role in the hierarchy can be skipped. Every assignment write — initial conversion *and* every subsequent reroute — emits a `task_delegations` audit row (see ADR-038). The disposition endpoint itself is unchanged; the audit trail becomes visible via `GET /tasks/:id/delegation-history`.
