...
## How it works (+ scenario matrix)

**[PROCESS.md](./PROCESS.md)** — the four processes end to end (rule→roster, punch→session, roster status lifecycle, the 3-axis presence model) and the **S1–S40 / M1–M7 scenario matrix** with expected roster status, lifecycle, flags, tone and counted-or-not for each. Use it as the UAT script and the test contract.
...
- **Roster reads carry the presence axes** (ADR-050) — `/schedules/range`, `/date/:date` and `/my/day` attach `lifecycle_state`, `lifecycle_flags`, `leave_reason`, `is_within_area`, `is_scheduled` via `RosterPresenceService`, reusing monitoring's `derivePresenceState`. Only rows dated **today or earlier** are derived (`lifecycle_state: null` on a future row = *not applicable*); sessions + tracking are each **one batched query** per request. Without this the board could only ever render 4 of the 9 ADR-050 tones.
- **Horizon** — `schedule.materialization_days` default **60** (a supervisor planning next month must see rows). The materializer's inline fallback previously said 30, contradicting the catalog and the ~60 days actually present on staging; both now read 60.
- **Absence sweep is bounded** — `schedule.absence_sweep_lookback_days` (default **7**, `0` = unbounded). Unbounded, the first sweep on a database that has never run ADR-056 rewrites the whole backlog in one transaction.
- **Operator tooling** — `apps/be/scripts/staging-verify-schedules.sql` (read-only drift report + the duplicate-place pre-flight gate) and `npm run schedules:to-daily` (promote standing one-offs to daily; dry-run by default).
