...
## Changelog
- 2026-07-12 — **Phase 1 review hardening.** Numeric settings now carry catalog **min/max bounds**, enforced on write (rate limits/thresholds can no longer be set to 0/negative/absurd values; the web number inputs expose the bounds); empty string no longer coerces to `0` for number settings; system-setting writes/clears are **audit-logged** (ADR-015, secret values masked as `***`); a `settings.changed` event drops the monitoring thresholds cache immediately (was: stale up to 60s TTL); web group-save un-stages each key as it persists so a mid-group failure leaves only truly-unsaved keys dirty; `DELETE /settings/:key` records the acting user.
