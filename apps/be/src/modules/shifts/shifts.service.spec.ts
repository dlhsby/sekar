...
describe('ShiftsService', () => {
  ...
  describe('getActiveArea', () => {
    it('should return area from the worker effective areas when one exists', async () => {
      mockUserLocationsService.getEffectiveAreas.mockResolvedValue([mockArea as any]);
      ...
      expect(mockUserLocationsService.getEffectiveAreas).toHaveBeenCalled();
    });

    it('should return null when the worker has no assigned area (ad-hoc)', async () => {
      mockUserLocationsService.getEffectiveAreas.mockResolvedValue([]);
    ...
    it('should pick the GPS-containing area among several candidates', async () => {
      ...
      mockUserLocationsService.getEffectiveAreas.mockResolvedValue([near as any, far as any]);
  ...
  describe('clockIn', () => {
    ...
    it('should successfully clock in with auto-detected area when location_id not provided', async () => {
      ...
      expect(result).toEqual(mockShift);
      expect(mockUserLocationsService.getEffectiveAreas).toHaveBeenCalled();
      expect(mockLocationsService.findOne).not.toHaveBeenCalled();
    });
