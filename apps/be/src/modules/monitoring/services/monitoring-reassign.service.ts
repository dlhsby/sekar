...
export class MonitoringReassignService {
  ...
  async reassign(dto: ReassignWorkerDto, requestingUser: User): Promise<ReassignWorkerResponseDto> {
    ...
    const targetArea = await this.areaRepository.findOne({
      where: { id: dto.target_location_id },
    });

    if (!targetArea) {
      throw new NotFoundException(`Target area ${dto.target_location_id} not found`);
    }
    ...
    worker.location_id = dto.target_location_id;
    await this.userRepository.save(worker);

    await this.trackingRepository.update(
      { user_id: worker.id },
      { location_id: dto.target_location_id },
    );
    ...
    const newScheduleId = await this.dailySchedulesService.overrideForDay(
      ...
      effectiveDate,
      {
        locationId: dto.target_location_id,
    ...
    this.eventsGateway.emitUserReassigned({
      ...
      previous_location_id: previousAreaId,
      previous_area_name: previousAreaName,
      new_location_id: targetArea.id,
    ...
    this.logger.log(
      `User ${worker.id} reassigned from area ${previousAreaId} to ${dto.target_location_id}`,
    );
