...
export class MonitoringStatsService {
  ...
  async getLocationStats(locationId: string): Promise<LocationStatsDto> {
    ...
    return {
      id: area.id,
      name: area.name,
      location_type: area.locationType?.name || 'Unknown',
      location_type_category: area.locationType?.category || 'active',
   ...
   * area. Location grouping goes through the schedule_locations join (a worker can be
  ...
  async getBoundaries(filters?: {
    ...
    const rayonBoundaries: RayonBoundaryDto[] = await Promise.all(
      rayons.map(async (rayon) => {
        ...
        // Korlap-style area scoping: when caller passes location_ids, only return
        ...
        if (filters?.location_ids) {
          ...
          areaWhere.id = In(filters.location_ids);
        }
        ...
        const areaBoundaries: LocationBoundaryDto[] = areas.map((area) => {
