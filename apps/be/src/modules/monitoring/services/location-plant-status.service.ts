...
/**
 * LocationPlantStatusService — Computes area-level plant maintenance status.
 *
 ...
 */
@Injectable()
export class LocationPlantStatusService {
  private readonly logger = new Logger(LocationPlantStatusService.name);
   ...
   */
  async getLocationPlantStatus(locationId: string): Promise<AreaPlantStatusResponse> {
   ...
   * Recomputes every row's status (same path as getLocationPlantStatus) and groups
