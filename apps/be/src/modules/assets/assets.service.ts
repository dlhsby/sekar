...
export class AssetsService {
  ...
  constructor(
    ...
    private userAreaRepo: Repository<UserLocation>,
    @InjectRepository(Location)
    private areaRepo: Repository<Location>,
  ...
  async findAll(user: User, query: QueryAssetDto): Promise<PaginatedResponseDto<Asset>> {
    ...
    if (query.location_id) {
      qb.andWhere('asset.location_id = :locationId', { locationId: query.location_id });
    }
    if (query.rayon_id) {
  ...
  private async applyScopeFilter(qb: any, user: User): Promise<void> {
    ...
    if (ASSET_VIEWERS.includes(role as any)) {
      if (role === UserRole.SATGAS || role === UserRole.LINMAS) {
        qb.andWhere('asset.location_id = :locationId', { locationId: location_id });
      } else if (role === UserRole.KORLAP) {
        const areas = await this.userAreaRepo.find({ where: { user_id: user.id } });
        const locationIds = areas.map((a) => a.location_id);
        if (locationIds.length > 0) {
          qb.andWhere('asset.location_id IN (:...locationIds)', { locationIds });
        }
  ...
  private async authorizeViewAsset(asset: Asset, user: User): Promise<void> {
    ...
    if (role === UserRole.SATGAS || role === UserRole.LINMAS) {
      ...
      const areas = await this.userAreaRepo.find({ where: { user_id: user.id } });
      const locationIds = areas.map((a) => a.location_id);
      if (asset.location_id && !locationIds.includes(asset.location_id)) {
  ...
  private async getAreaRayon(locationId: string): Promise<string> {
    const area = await this.areaRepo.findOne({ where: { id: locationId } });
