...
describe('UserLocationsService', () => {
  ...
  describe('getEffectiveLocations', () => {
    it('should return areas for user', async () => {
      ...
      const result = await service.getEffectiveLocations('user-uuid-1');
  ...
  describe('getPermanentLocations', () => {
    it('should return only permanent assignments', async () => {
      mockUserAreaRepo.find.mockResolvedValue([mockUserArea]);

      const result = await service.getPermanentLocations('user-uuid-1');
  ...
  describe('assignLocations', () => {
    it('should create new assignments', async () => {
      ...
      const result = await service.assignLocations('user-uuid-1', ['area-uuid-1'], 'admin-uuid');
    ...
    it('should throw NotFoundException if user not found', async () => {
      ...
      await expect(
        service.assignLocations('nonexistent', ['area-uuid-1'], 'admin-uuid'),
    ...
    it('should throw NotFoundException if area not found', async () => {
      ...
      await expect(
        service.assignLocations('user-uuid-1', ['nonexistent-area'], 'admin-uuid'),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('getUsersByLocation', () => {
    it('should return users for an area', async () => {
      ...
      const result = await service.getUsersByLocation('area-uuid-1');
  ...
  describe('isUserAssignedToLocation', () => {
    it('should return true if assigned', async () => {
      mockUserAreaRepo.count.mockResolvedValue(1);

      const result = await service.isUserAssignedToLocation('user-uuid-1', 'area-uuid-1');
    ...
    it('should return false if not assigned', async () => {
      ...
      const result = await service.isUserAssignedToLocation('user-uuid-1', 'area-uuid-2');
