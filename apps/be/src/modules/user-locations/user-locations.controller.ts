...
export class UserLocationsController {
  ...
  @ApiResponse({ status: 200, description: 'List of assigned locations' })
  async getUserLocations(@Param('userId', ParseUUIDPipe) userId: string) {
    return this.userLocationsService.getEffectiveLocations(userId);
  }

  @Post('users/:userId/areas')
  @Roles(...USER_MANAGERS, UserRole.KEPALA_RAYON)
  @ApiOperation({ summary: 'Assign locations to user' })
  @ApiResponse({ status: 201, description: 'Locations assigned' })
  async assignLocations(
    @Param('userId', ParseUUIDPipe) userId: string,
    @Body() dto: AssignLocationsDto,
    @GetUser() currentUser: User,
  ) {
    return this.userLocationsService.assignLocations(userId, dto.location_ids, currentUser.id);
  }

  @Delete('users/:userId/areas/:locationId')
  @Roles(...USER_MANAGERS, UserRole.KEPALA_RAYON)
  @ApiOperation({ summary: 'Remove location assignment' })
  ...
  async getLocationUsers(@Param('locationId', ParseUUIDPipe) locationId: string) {
    return this.userLocationsService.getUsersByLocation(locationId);
  }
}
