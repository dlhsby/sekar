...
import { AreaStaffRequirement, DayType } from './entities/area-staff-requirement.entity';
...
@Controller('areas/:areaId/staff-requirements')
@UseGuards(JwtAuthGuard, RolesGuard)
export class AreaStaffRequirementsController {
  constructor(private readonly staffRequirementsService: AreaStaffRequirementsService) {}
  ...
  findByArea(@Param('areaId') areaId: string): Promise<AreaStaffRequirement[]> {
  ...
  getSummary(
    ...
    return this.staffRequirementsService.getRequirementsSummary(areaId, dayType);
  }
  ...
  findOne(@Param('id') id: string): Promise<AreaStaffRequirement> {
  ...
  @ApiResponse({
    status: 404,
    description: 'Area or shift definition not found',
  })
  @ApiResponse({
    status: 409,
  ...
  create(
    @Param('areaId') areaId: string,
    @Body() createDto: CreateAreaStaffRequirementDto,
  ): Promise<AreaStaffRequirement> {
    // Override area_id from URL parameter
    return this.staffRequirementsService.create({ ...createDto, area_id: areaId });
  }
  ...
  update(
    @Param('id') id: string,
    @Body() updateDto: UpdateAreaStaffRequirementDto,
  ): Promise<AreaStaffRequirement> {
