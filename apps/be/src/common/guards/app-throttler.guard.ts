 ...
 *   - `login`          → `ratelimit.login_per_min`
 *   - `changePassword` → `auth.change_password_throttle_max` / `_ttl_ms`
 *   - everything else  → `ratelimit.global_per_min`, but ONLY when the incoming
 *     limit still equals the module default (i.e. no route-level `@Throttle`
 *     override such as `refresh`'s 10/min set it).
 *
  ...
  protected handleRequest(requestProps: ThrottlerRequest): Promise<boolean> {
    ...
    try {
      ...
      if (handler === 'login') {
        ...
        const moduleDefault = this.throttlers?.[0]?.limit;
        if (typeof moduleDefault === 'number' && limit === moduleDefault) {
