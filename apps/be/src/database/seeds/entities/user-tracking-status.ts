...
/**
 * Seed user tracking status (mode-dependent):
 *
...
export async function seedUserTrackingStatus(ctx: SeedContext): Promise<void> {
  ...
  if (ctx.mode === 'staging') {
    ...
    // DEMO: Backfill user_tracking_status for all clockable users (offline default)
    // ==========================================
    // D.2: Backfill user_tracking_status for all clockable users (offline default)
    // ==========================================
    const clockableUsers = (await ctx.qr.query(`
      SELECT id, location_id FROM users
      WHERE role IN ('satgas', 'linmas', 'korlap', 'admin_data') AND deleted_at IS NULL
    `)) as Array<{ id: string; location_id: string | null }>;

    for (const user of clockableUsers) {
      await ctx.qr.query(
        `INSERT INTO user_tracking_status (user_id, status, location_id, updated_at)
    ...
    for (const user of monitoringUsers) {
      ...
      switch (status) {
        ...
        case 'outside_area':
          ...
          lng = user.gps_lng
            ? parseFloat(user.gps_lng as any) + (Math.random() - 0.5) * 0.1
            : 112.8;
          isWithinArea = false;
          break;
