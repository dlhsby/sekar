 ...
 * `location_staff_requirements` table: region-level for the 7 grouped districts'
 * kawasan, location-level for Taman Aktif parks (per each district's
...
export async function seedStaffingRequirements(ctx: SeedContext): Promise<void> {
  ...
  for (const r of rows) {
    ...
    const districtId = r.subject_type === 'district' ? r.subject_id : null;
    await ctx.qr.query(
      `INSERT INTO location_staff_requirements
         (id, location_id, region_id, district_id, shift_definition_id, role, required_count, day_type)
