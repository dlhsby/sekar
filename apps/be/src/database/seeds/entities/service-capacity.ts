...
/**
 * Seed service_capacity — pruning booking grid for 7 districts × 12 ISO weeks.
 ...
 * Rotation offset per district so different districts show different load profiles.
 */
export async function seedServiceCapacity(ctx: SeedContext): Promise<void> {
  ctx.log('📅 Seeding Service Capacity…');

  const districts = await ctx.qr.query(`SELECT id, name FROM districts ORDER BY name`);
  if (districts.length === 0) {
    ctx.log('  ⚠ No districts found, skipping service_capacity seed');
    return;
  }
  ...
  // Booked pattern rotates by district so different districts show different load profiles:
    ...
    for (let i = 0; i < weeksToSeed.length; i++) {
      ...
      const result = await ctx.qr.query(
        `INSERT INTO service_capacity
            (district_id, year, iso_week, service_type, capacity_units, booked_units)
         VALUES ($1, $2, $3, 'pruning', $4, $5)
         ON CONFLICT (district_id, year, iso_week, service_type) DO UPDATE SET
            capacity_units = EXCLUDED.capacity_units,
            booked_units   = EXCLUDED.booked_units
         RETURNING (xmax = 0) AS inserted`,
        [district.id, year, week, CAPACITY_UNITS, booked],
      );
  ...
  ctx.log(
    `  ✓ ${capacityInserted} new + ${capacityUpdated} refreshed service_capacity rows ` +
      `(${districts.length} districts × ${weeksToSeed.length} weeks, capacity_units=${CAPACITY_UNITS}, ` +
