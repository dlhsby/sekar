 ...
 *
 * Rooms:
 * - area:{locationId} - Subscribers to specific area
...
export class EventsGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  afterInit(server: Server): void {
    ...
    if (this.staffingDebouncer) {
      this.staffingDebouncer.setEmitter((locationId, state) => {
  ...
  emitUserReassigned(event: UserReassignedEvent): void {
    ...
    if (event.previous_location_id) {
      this.server
        .to(`monitoring:area:${event.previous_location_id}`)
        .emit(EventType.USER_REASSIGNED, event);
    }
    this.server
      .to(`monitoring:area:${event.new_location_id}`)
      .emit(EventType.USER_REASSIGNED, event);
    if (event.rayon_id) {
