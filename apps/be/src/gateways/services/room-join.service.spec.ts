...
describe('RoomJoinService', () => {
  ...
  let userLocationsService: { getPermanentLocationIds: jest.Mock };

  beforeEach(async () => {
    userRepository = { findOne: jest.fn().mockResolvedValue(null) };
    userLocationsService = { getPermanentLocationIds: jest.fn().mockResolvedValue([]) };

    const module: TestingModule = await Test.createTestingModule({
      providers: [
        RoomJoinService,
        { provide: getRepositoryToken(User), useValue: userRepository },
        { provide: UserLocationsService, useValue: userLocationsService },
      ],
    }).compile();
  ...
  describe('getRoomsForUser', () => {
    ...
    it('should include all assigned area rooms for korlap (multi-area)', async () => {
      ...
      userLocationsService.getPermanentLocationIds.mockResolvedValue(['a1', 'a2']);
