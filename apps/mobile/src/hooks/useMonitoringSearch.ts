...
export interface SearchResult {
  ...
  name: string;
  /** Secondary line — role · location for petugas, parent rayon for location, area count for rayon. */
  subtitle?: string;
  latitude: number;
  longitude: number;
...
export function useMonitoringSearch(
  ...
  query: string,
  labels?: { petugas: string; location: string; rayon: string },
  ...
  return useMemo(() => {
    ...
    const petugas: SearchResult[] = liveUsers
      ...
      .map((u) => ({
        ...
        name: u.full_name,
        subtitle: [roleLabel(u.role), u.area_name].filter(Boolean).join(' · '),
        latitude: u.latitude,
    ...
    const semua: SearchSection[] = [
      ...
      { title: labels?.location ?? 'Area', type: 'location' as const, data: location },
