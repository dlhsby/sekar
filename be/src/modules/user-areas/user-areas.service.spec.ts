...
describe('UserAreasService', () => {
  ...
  const mockUserArea: Partial<UserArea> = {
    id: 'ua-uuid-1',
    user_id: 'user-uuid-1',
    area_id: 'area-uuid-1',
  ...
  beforeEach(async () => {
    ...
    service = module.get<UserAreasService>(UserAreasService);
  });
  ...
  describe('syncTaskBasedAreas', () => {
    it('should remove old task-based areas and add new ones', async () => {
      ...
      await service.syncTaskBasedAreas('user-uuid-1', ['area-uuid-2']);
    ...
    it('should skip areas that already have any assignment', async () => {
      ...
      await service.syncTaskBasedAreas('user-uuid-1', ['area-uuid-1']);
