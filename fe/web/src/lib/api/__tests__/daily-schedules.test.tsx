...
describe('Daily Schedules API', () => {
  ...
  describe('Query Keys', () => {
    it('should generate correct query keys', () => {
      expect(dailyScheduleKeys.all).toEqual(['daily-schedules']);
      expect(dailyScheduleKeys.lists()).toEqual(['daily-schedules', 'list']);
  ...
  describe('useDailyRoster', () => {
    it('should fetch daily schedules for a date', async () => {
      ...
      mockAxios.onGet(`/daily-schedules/date/${date}?rayonId=`).reply(200, [mockDailySchedule]);
    ...
    it('should fetch daily schedules filtered by rayon', async () => {
      ...
      mockAxios
        .onGet(`/daily-schedules/date/${date}?rayonId=${rayonId}`)
        .reply(200, [mockDailySchedule]);
    ...
    it('should handle empty roster', async () => {
      const date = '2026-02-04';
      mockAxios.onGet(`/daily-schedules/date/${date}?rayonId=`).reply(200, []);
  ...
  describe('useMyRoster', () => {
    it('should fetch current user roster', async () => {
      mockAxios.onGet('/daily-schedules/my?').reply(200, mockDailySchedule);
    ...
    it('should fetch user roster for specific date', async () => {
      mockAxios
        .onGet('/daily-schedules/my?date=2026-02-04')
        .reply(200, mockDailySchedule);
    ...
    it('should handle null roster', async () => {
      mockAxios.onGet('/daily-schedules/my?').reply(200, null);
  ...
  describe('useGenerateRoster', () => {
    it('should generate roster for a date', async () => {
      mockAxios.onPost('/daily-schedules/generate').reply(200, { generated: 10 });
    ...
    it('should handle generation errors', async () => {
      mockAxios.onPost('/daily-schedules/generate').reply(400, { error: 'Bad request' });
  ...
  describe('useSetLeave', () => {
    it('should set leave on roster entry', async () => {
      ...
      mockAxios.onPatch(`/daily-schedules/daily-1/leave`).reply(200, updatedSchedule);
  ...
  describe('useReplaceWorker', () => {
    it('should replace worker on roster entry', async () => {
      ...
      mockAxios.onPatch(`/daily-schedules/daily-1/replace`).reply(200, updatedSchedule);
  ...
  describe('useUpdateRosterAreas', () => {
    it('should update areas on roster entry', async () => {
      ...
      const updatedSchedule: DailySchedule = {
        ...mockDailySchedule,
        daily_schedule_areas: [
          mockDailySchedule.daily_schedule_areas[0],
          area2,
        ],
      };
      mockAxios.onPatch(`/daily-schedules/daily-1/areas`).reply(200, updatedSchedule);
      ...
      expect(result.current.data?.daily_schedule_areas).toHaveLength(2);
    });
  });
  ...
  describe('useUpdateRosterShift', () => {
    it('should update shift on roster entry', async () => {
      ...
      mockAxios.onPatch(`/daily-schedules/daily-1/shift`).reply(200, updatedSchedule);
