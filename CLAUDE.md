...
- **Backend env lives in `be/`** (consistent with the fe/* workspaces): `be/.env.staging` (baked into the staging image) and `be/.env.production` (drives the root `docker-compose.prod.yml` via `env_file: be/.env.production`). No root-level app env. Run prod with `docker compose --env-file be/.env.production -f docker-compose.prod.yml …`.
