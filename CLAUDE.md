...
**Last Updated:** April 27, 2026 (M3 + M4 admin finish-out + 3-11 capacity + 3-12 plant-seeds backend landed)
**Status:** Phase 2E ✅ + Monitoring Map Bugfixes ✅ | Phase 3 IN PROGRESS — M1-R ✅ + M1-S ✅ + M2 ✅ + M3 ✅ + **M4 ✅** (3-9 ✅ both halves, 3-10 ✅ kecamatan + admin, 3-11 ✅ capacity backend) + **3-12 🟡** (backend + redux scaffold complete; mobile inventory screens deferred to Phase 4 polish). 17/21 sub-phases (~81%). **Backend coverage:** `PlantsController` (5 endpoints, 41 tests, 100/97%) + `PruningRequestsController` (6 endpoints, 55 tests, 99.35/93.1%) + `ServiceCapacityController` (3 endpoints, 24 tests, 98.9/93.3%) + `PlantSeedsController` (5 endpoints, 29 tests, 100/94.23%). **Mobile:** kecamatan submit + admin review + convert-to-task all live; plant-seeds slice + API ready (screens deferred). **Bug fix (Apr 27):** SubmitScreen runtime crash (`nbColors.bg.primary undefined`) fixed in `9f343c7` by mapping all nested token paths to flat names + regression-guard test in `nbTokens.test.ts`. Web work (pruning queue, capacity UI, seeds pages, dynamic task forms) + 3-13 CSV backfill + 3-14 k6 load test + 3-8 cron/FCM/map overlay all explicitly deferred. ADRs 029–037 Accepted. Next: 3-12 mobile screens + 3-13 backfill + 3-14 load test + web finish-out + Phase 4.
