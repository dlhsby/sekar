...
**Status:** Phase 2E Complete ✅ + Monitoring Map Bugfixes ✅ | Phase 3 IN PROGRESS — **M1-R ✅** (5/5), **3-1 ✅**, **3-2 ✅** (8 entities, `staff_kecamatan` role, 128 plant species), **M2 Monitoring v2 ✅** (3-3 backend + 3-4 web + 3-5 mobile, Apr 26). **M2 backend deployed to production Apr 27** — Phase 3 schema applied + reference data seeded on `api.sekar.wahyutrip.com`; Redis provisioning deferred (Upstash/ElastiCache options documented in `phase-3-deployment.md`). **Apr 27 polish pass:** mobile collapsible selfie cards (clock-in/out + lembur), `Detail per Peran` → `Detail` rename in monitoring, mobile coverage thresholds locked at current floor (75/68/70/76), web coverage cleared to 80 % (sw/** excluded), `db:seed:prod` + `db:seed:staging:prod` now bundle Phase 3 reference + UAT sample data so cold-starts and UAT resets are one-step. **Next: 3-6 (Task typing + partial-complete API)**. ADRs 029–037 Accepted. Phase 4/5 renumbered.
