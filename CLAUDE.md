...
**Last Updated:** April 27, 2026 (M3 + M4 mobile spine landed)
**Status:** Phase 2E ✅ + Monitoring Map Bugfixes ✅ | Phase 3 IN PROGRESS — M1-R ✅ + M1-S ✅ + M2 ✅ + **M3 mobile spine ✅ (3-6 partial-complete + 3-7 plants form/autocomplete + 3-8 light due-date chip)** + **M4 mobile spine 🟡 (3-9 submit-half + 3-10 kecamatan slice)**. 13/21 sub-phases. **M2 backend on prod Apr 27.** **Apr 27 M3+M4 mobile-spine:** backend `PlantsController` (5 endpoints, 41 tests) + `PruningRequestsController` (3 endpoints, 30 tests) + `PlantDueDateService` + `AreaPlantStatusService` + endpoint `GET /monitoring/area/:id/plant-status`; mobile `PartialCompleteSheet`, `SpeciesAutocomplete`, `PruningTaskForm`, `PlantStatusChip`, `KecamatanNavigator` (role-gated), 5-step `SubmitScreen`, `MyRequestsScreen`, `RequestDetailScreen`, `plantsSlice`, `pruningRequestsSlice`, offline-queue scaffold for `pruning_request.submit`. Web work for 3-7/3-10 + admin endpoints (review/convert/disposition) + 3-11 capacity calendar + 3-12 plant-seeds + 3-8 cron/FCM/map overlay all explicitly deferred to next iteration. Mobile coverage thresholds 75/68/70/76 hold. **Code review (12 findings: 4 critical + 6 medium + 2 low) all fixed same-day in `ff7d128`** — see `status_reviews.md` "M3+M4 Mobile Slice Review". Next: M3+M4 admin finish-out (review/convert/disposition endpoints + ReviewQueueScreen + ConvertToTaskSheet + capacity calendar + plant-seeds inventory). ADRs 029–037 Accepted.
