...
# SEKAR Database Startup Script
# This script starts PostgreSQL and Adminer using Docker Compose
# Run from project root: ./db/start.sh
...
# Change to db directory to run docker-compose
cd "$SCRIPT_DIR"
...
echo -e "  • Stop services: ${GREEN}./db/stop.sh${NC} or ${GREEN}cd db && docker-compose down${NC}"
echo -e "  • View logs: ${GREEN}cd db && docker-compose logs -f${NC}"
echo -e "  • View status: ${GREEN}cd db && docker-compose ps${NC}"
echo -e "  • Access PostgreSQL CLI: ${GREEN}cd db && docker-compose exec postgres psql -U $DB_USER -d $DB_NAME${NC}"
echo ""
